...
import { login, login360, loginNET, setTermsAndConditions, workflowstart, paywayConfirm, loginZup } from '../../requests/loader';
...
class Login extends Component {
    ...
    getErrors() {
        ...
        if (this.isNetVendor()) {
            ...
            errors['email'] = Translator.get('invalid_username_length', 'La longitud de la usuario no debe ser inferior a 5 caracteres')
        }
        /*else if (formValues.email.length > 1 && formValues.email.includes("@") === false) {
            errors['email'] = null
        }*/
        else if (formValues.email.includes("@") === true && !/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i.test(formValues.email)) {
                errors['email'] = Translator.get('registro_valido_mail', 'Debes completar el campo con un correo electrónico válido');
        }
    ...
    setCaptcha() {
        ...
        console.log("url_imagen", showCaptchaTask.getImageUrl())
        const captcha = {
            url,
            show: true
        }
    ...
    async handleSubmit() {
        ...
        console.log("login_password", params.password.length)
    ...
    async performLogin(params = {}) {
        ...
        // HACK goose -- ZUP login
        this.setState({vendor:"VENDOR_ZUP"});
        loginFunction=loginZup;

        try {
            const result = await loginFunction(params);
            // console.info("zup***** resulT a la vuelta \n",JSON.stringify(result,"\n"));
    ...
    goToPassword = () => {
        this.props.history.push(`/password-reminder/`);
    }
    ...
    showModal(type, msg, props){
        ...
        const formValues = this.props.formValues;
        let password_key = {}
        password_key.code = "net_login_usuario_pass_obligatorio";
    ...
    render() {
        ...
        if(this.userLoged && window.location.pathname === '/login')
          this.props.history.push('/');
        if (this.state.isLoading) return <LoadingComponent title={Translator.get('loading', 'carregando')} image background="#030c14" /> ;
        else
        console.log("img catch", this.state.captcha.url)
        return (
               ...
               {/* <div className="form-group__input-tab-sesion" id="form-tab-sesion">
                ...
                <div className="login-inputs">
                    <div id="email" className="tabcontent">
                        <h2 className="login-labels">{Translator.get('net_login_user', 'Usuário ou email')} <span className="contenidos contenidos-email" style={{display:"none"}}>*</span></h2>
                        {this.props.renderGroup('email', this.isNetVendor() ? 'Login':  Translator.get('ingresar_email', 'Correo electrónico'))}

                        <div className="login-password-input">
                            <h2 className="login-labels">{Translator.get('net_login_pass', 'digite sua senha')} <span className="contenidos contenidos-password" style={{display:"none"}}>*</span></h2>
                            ...
                            : 
                            <div class="form-group form-group__input-password">
                                <a class="form-group-focus form-group-focus-password_hiden false disabled" href="javascript:void(0)">
                                    <div class="form-group-value form-group-value-password_hiden ">
                                        <div class="form-group-placeholder">{Translator.get('ingresar_password', 'Contraseña')}</div>
                                        <a class="form-group__button-show-password password-input null disabled" href="javascript:void(0)">MOSTRAR</a>
                                    </div>
                                </a>
                            </div>
                            } 
                ...
                <div className="form-group__register-telcel">
                    ...
                    <div className="login-btn-container">
                        <a 
                            className={this.props.formValues.password.length > 0 ? `btn-login focusable` : `btn-login disabled`} 
                            href="javascript:void(0)"
                        ...
                        >
                            {Translator.get('registro_olivide_contrase\u00f1a', 'esqueceu a senha')}
                        </a> */}
                    </div>
