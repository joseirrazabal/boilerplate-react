...
class RibbonGrid extends Component {
  ...
  handleFocus(data) {
    ...
    if(Utils.getToggleFunctionsFromMetadata().scrolling_ribbon_home){
      let item=document.getElementById(document.activeElement.getAttribute("id")).parentNode;
      while( item.getAttribute("class") != "special-ribbon-carousel" ){
        item=item.parentNode;
      }
      let position = item.parentNode.offsetTop;
      document.querySelector("#slider-grid-movies .scroller-container").style.marginTop = `-${position}px`;
    }
  ...
  levelUserRequest(params, callback) {
    if (this.props.levelUserRequest.params.user_hash !== '') {
      ...
      const newParams = Object.assign({}, this.props.levelUserRequest.params, params);
    ...
    */
    ...
    return (
      <div className="ribbongrid-wrapper">
        <div className="ribbongrid-container" id={Utils.getToggleFunctionsFromMetadata().scrolling_ribbon_home ? "slider-grid-movies" : "" }>
          <Scrollable height={'410px'}>
            {
              ribbons.map((ribbon) => {
                ...
                /* Se elimina separacion del nodo por ver
                if (id.includes('porver')) {                          

                  let reduxItems = [];
                  const groups = ribbon.subtype === 'vod' ? store.getState().user.groups : store.getState().user.channels_groups;

                  if (Utils.isArray(groups)) {
                      reduxItems = [];
                      groups.map((item) => {
                        reduxItems.push(RibbonsUtil.parserItem(item));
                      });
                  }

                  ribbon.items = reduxItems;

                }
                */
                return <SpecialRibbon
