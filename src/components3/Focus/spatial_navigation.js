...
;(function($) {
  ...
  function onKeyDown(evt) {
    ...
    if (fireEvent(currentFocusedElement, 'willmove', willmoveProperties) && Utils.canMoveFocus(evt.keyCode)) {
  ...
  function deviceIsNotSamsung() {
    return Device.getDevice().getPlatform() === 'samsung'? false : true;
  }
  ...
  function onKeyUp(evt) {
    ...
    if (!_pause && _sectionCount && evt.keyCode === 13 && deviceIsNotSamsung()) {
      var currentFocusedElement = getCurrentFocusedElement();
      if (currentFocusedElement && getSectionId(currentFocusedElement)) {
  ...
  var SpatialNavigation = {
    init: function() {
      if (!_ready) {
        ...
        if ( navigator.userAgent.toLowerCase().indexOf("opera") !== -1 && Utils.deviceIsNotHisense() ) {
