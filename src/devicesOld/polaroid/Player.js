...
import {DASHWV,HLS_KA,DASHWV_MA} from "../../utils/playerConstants";
import Utils from '../../utils/Utils';

/* TODO this must not inherit from HTML5 */
class PolaroidPlayer extends AbstractHTML5Player {
  ...
  processPlayerMedia(){
    if(this.currentPlayerMedia && !this.streamIsAudio()) {
      try {
        ...
        let isMA = videoOptions.streamType ? videoOptions.streamType === DASHWV_MA : false;
  ...
  replaceMediaSource(newSource) {
    ...
    var isNewSourceMA = newSource.streamType ? newSource.streamType === DASHWV_MA : false;
    var isThisSourceMA = this.options.streamType ? this.options.streamType === DASHWV_MA : false;
    ...
    this.options.contentId = newSource.drmInfo.content_id ? newSource.drmInfo.content_id : null;
  ...
  _onStreamError(param){
    ...
    let streamType = isLive ? "hls" : "dashwv";
