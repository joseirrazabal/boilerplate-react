...
class ModalWrapper extends PureComponent {
    ...
    renderButtons(buttons = []) {
        const _this = this;
        let test = this.props.showModalConfirmation === false ?  buttons.map(button => {
    ...
    render() {
        ...
        if (type == "string" && this.props.content.includes(".") && this.props.content.length > 60 && !urlRegExp.test(this.props.content) && !acronym.test(this.props.content)) {
            ...
            content.push(this.renderOptionalChild('content', <p className="modal_text" dangerouslySetInnerHTML={this.getHtml(msg)} />));
        }
        else {
...
const mapStateToProps = state => ({ 
    showModalConfirmation: state.players.showPlayerProps
});
