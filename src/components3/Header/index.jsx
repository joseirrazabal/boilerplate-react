...
class Header extends Component {
  ...
  constructor(props) {
    ...
    console.log("javier nodes", navcodes)
  ...
  getAllNodes(nodes) {
    ...
    nodes.forEach(node => {
      ...
      if (node.childs && node.childs.length > 0) {
        ...
        console.log("javier nodes", childs)
        allNodes = [...allNodes, ...childs];
        console.log("javier node 2", allNodes)
      }
    }, this);
    return allNodes;
  }
      ...
      if (this.mapa[node].childs) {
        ...
        if (this.mapa[node].parent === "__root" || !this.mapa[node].parent) {
         ...
         // forDown = this.mapa[node].childs;
          forSubmenu = this.mapa[node].childs;
          forTop = [];
          forDown = this.props.nodes;
          console.log("JAVIER",this.props.nodes)
      ...
      () => {
        if (
          ...
          document.getElementById(
            "header-down"
          ).firstChild.firstChild.style.marginLeft = `-${this
            .menuOnFocusPosition}px`;
        }
      }
    );
  }
  ...
  getLinks() {
    ...
    console.log("javier node 60",links)
    ...
    /*if(isOnDown && this.mapa[cNode] && this.mapa[cNode].parent)
   {
    ...
    switch (direction) {
      case "up":
        ...
        */
        if (
          this.props.history.location.pathname.indexOf("nagrainicio") !== -1 ||
  ...
  setSelected(selected = {}) {
    ...
    if (prev.id !== selected.id) {
      if (prevDepth > selectedDepth) {
        // if selected upper level
        this.tree.length = selectedDepth + 1;
      } else if (prev.id === selected.id_parent) {
        // if child selected
      ...
      if (children) {
        ...
        this.setState({
          top: [],
          down:
            this.tree && this.tree[selectedDepth]
              ? this.tree[selectedDepth]
              : []
        });
      }
  ...
  handleShowPerfilModal(isHuaweiArgentina = false) {
    ...
    if (!isHuaweiArgentina && this.props.user.isLoggedIn) {
      UtilsIpTelmex.validateMail(
        this.props.user.session_userhash
      ).then(function(resp) {
  ...
  NavItem = ({ node, index, setSelected, actives, position }) => {
    console.log("javier 423", node, index, setSelected, actives, position)
    let nodeValue;
    let iconMenu = Assets.get(`ic_imgmenu_${node.code}`);
    console.log("javier img", iconMenu)
    ...
    console.log("javier level",level)
          ...
          if (this.menuWidth > 1280) {
            this.menuOnBlur = setTimeout(() => {
              ...
              if (marginLeft !== null) {
                document.getElementById(
                  idItem
                ).parentNode.style.marginLeft = `-${this
                  .menuOnFocusLastPosition}px`;
              }
            }, 400);
          }
        }}
  ...
  render() {
    console.log("JAVIER 526", this.state.subMenu);
    ...
    console.log("JAVIER",down);

    const clockOrNav = (
      /* this.state.top.length === 0 ? (
        <div className="clock">
          <Clock />
          {!Device.getDevice().getConfig().hideArrowHeader && (
        ...
        >
          <div id="header-top" className="header-top">
            ...
            {showByPTV && (
              ...
              {
                headerRightContent.map(item => {
                  return (
                    <span   
                      onClick={() => this.setState({ icon : item.key })}
                      className={this.state.icon === item.key 
                      ? "header-right-icons active":"header-right-icons"} key={item.key}>
                      {item}
                    </span>
                  )
                })
