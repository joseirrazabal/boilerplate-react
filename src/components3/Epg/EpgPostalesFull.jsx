...
import {showModal, MODAL_ACTION, HIDE_MODAL, MODAL_CHANNEL_EPG, MODAL_EPG} from '../../actions/modal';
...
class Epg extends EpgLogic {
  ...
  calcInitRangeTime() {
    ...
    time.seconds = 0;
    time.milliseconds = 0;
    this.memory_range.horizontal.start = moment(time).subtract(this.props.visibleTime * this.props.horizontalOffset, 'hours');
    this.memory_range.horizontal.end = moment(this.memory_range.horizontal.start).add(this.props.horizontalOffset * this.props.visibleTime * 2 + this.props.visibleTime, 'hours');
    this.memory_visible.horizontal.start = moment(time);
    this.memory_visible.horizontal.end = moment(this.memory_visible.horizontal.start).add(this.props.visibleTime, 'hours');
  }
  ...
  request(params = {}, callback = (response) => {}) {
    console.log('[EPG] -- epgPostalesFull!!! ');
    ...
    if (!this.isRequesting) {
      try {
        ...
        promise.then(function(response) {
          ...
          if (settings.epg_cache_enabled) {
            ...
            epgCache.set(paramsQueryString, JSON.stringify(response), this.epgCacheExpireTimeSeconds);
          }
          ...
          if (this && this.props && this.props.hideEpgOutside && this.props.hasEpgError) {
      ...
      console.log('IPIGI ******** se pierde un request this.props:', this.props );
    }
  }
  ...
  firstLoad(customParams = {}) {
    console.log('IPIGI fullTohide en postales firstLoad customParams:', customParams);
    if(this.channelsContainer)
      this.channelsContainer.innerHTML=null;
    if(this.eventContainer)
      this.eventContainer.innerHTML=null;
    this.getChannelData();
    const params = Object.assign({}, {
      ...
      horizontal_blocks: (this.props.visibleTime * this.props.horizontalOffset) + 1,
    ...
    this.request(params, function (response) {
      ...
      console.log('IPIGI fullTohide en postales firstLoad, regreso del request response: ', response);


      if (response.body) {
        this.minuteSize = response.constants ? response.constants.minute_size : null;
        this.rowWidth = response.constants ? response.constants.row_width : null;
        this.total = response.total;
        //console.log('EPGFILTER Epg render en firstload de postales, en 1 total es ', this.total, params);
        if (!this.total) {
          //console.log('EPGFILTER Epg render en firstload de postales, en 2 total es ', this.total, params);
          if (this.eventContainer) {
            //console.log('EPGFILTER Epg render en firstload de postales, en 3 total es ', this.total, params);
            this.eventContainer.innerHTML = `<p id="epg-nodata">${Metadata.get("epg_nodata", "No hay canales disponibles")}</p>`;
          }
          ...
          this.forceFill = true;
          this.fromNewRequest = this.getFromFiltered(response.entry.node_id,response.lastChannel);
        }

        const {events, channels, data} = this.parseResponse(response);

        this.currentHorizontalIndex = this.props.horizontalOffset;
        this.currentVerticalIndex = this.isGoingToFetch ? this.props.verticalOffset / this.props.visibleRows : 0;
        ...
        this.memory_visible.vertical.start = nextFrom > this.total ? nextFrom - this.total : nextFrom; //nextFrom == this.total ? nextFrom + 1 : nextFrom;
        ...
        const active = document.querySelector('.focusable.epg-event-current.epg-event-item-container');
        ...
        if (typeof this.props.onFirstLoad === 'function' && element && !this.props.isFullEpg) {
          ...
          if (channel) {
            channel=channel.reduce(channel=>channel);
            const event = channel.events[this.currentHorizontalIndex].find((event) => event.id === eventId);
  ...
  setCurrentData() {
    try {
      ...
      if (focused) {
        ...
        let channel = this.epg.data[this.currentVerticalIndex] && this.epg.data[this.currentVerticalIndex].find((channel) => channel && channel.id === channelId);
        if (!channel) {
        ...
        if (channel) {
          if(channel.events[this.currentHorizontalIndex]) {
            let event = channel.events[this.currentHorizontalIndex].find((event) => event.id === eventId);
            ...
            console.error('RDGV: iVertical 0> currentVerticalIndex: ', this.currentVerticalIndex, ' iHorizontal', this.currentHorizontalIndex, ' data', this.epg.data);
          }
        } else {
          //TODO: ¿Aún pasa lo de los indices?
          console.error('RDGV: iVertical 1> currentVerticalIndex: ', this.currentVerticalIndex, ' iHorizontal', this.currentHorizontalIndex, ' data', this.epg.data);
        }
      }
    } catch (error) {
      //TODO: ¿Aún pasa lo de los indices?
      console.error('RDGV: iVertical 2> currentVerticalIndex: ', this.currentVerticalIndex, ' iHorizontal', this.currentHorizontalIndex, ' data', this.epg.data);
    }
  }
  ...
  removeListeners() {
    ...
    if (this.eventContainer) {
      ...
      this.eventContainer.removeEventListener('sn:navigatefailed', this.changePageHandler);
  ...
  addListeners() {
    ...
    if (this.eventContainer) {
      ...
      this.eventContainer.addEventListener('sn:navigatefailed', this.changePageHandler);
  ...
  handleClick = () => {
    ...
    store.dispatch(navigateFrom('FullEPG'));
  ...
  showModalOTT=()=>{
    ...
    const type_user_menu = (this.props.user.is_user_logged_in) ? this.props.user.is_user_logged_in : 0;
  ...
  keyPressListener(e) {
    ...
    console.log('Enter to Listening [EPGPostales] keyPressListener, currentKey:', currentKey);
    switch (currentKey) {
      case  'OK':
        if(!this.handleDelayKeyOkTimer && LocalStorage.getItem('config_remote_control')==='simple'){
          if(this.props.isFullEpg)
            ...
            this.handleDelayKeyOk(e, currentKey,this.cbOk);
        }
        break;
      case 'GUIDE':
        e.preventDefault();
  ...
  shouldComponentUpdate(nexprops, nextstate) {
    if (JSON.stringify(nexprops)!=JSON.stringify(this.props) || JSON.stringify(nextstate)!=JSON.stringify(this.state))
    {
      return true
    }
      return false
  ...
  changePageHandler(e) {
    ...
    switch (direction) {
      ...
      case this.leftDirection:
        const nextTimeshift = this.now().diff(this.memory_visible.horizontal.start, 'minutes');
      ...
      case this.downDirection:
        if (++this.currentVerticalIndex > this.epg.events.length - 1) {
          this.currentVerticalIndex = 0;
        }
        break;
      case this.upDirection:
        if (--this.currentVerticalIndex < 0) {
  ...
  nextRenderHandler(direction) {
    ...
    }else if (this.isHorizontalDirection(direction) && this.epg.events[this.currentVerticalIndex][0][this.currentHorizontalIndex]) {
      this.renderNextEvents(direction);
    } else if (this.isVerticalDirection(direction) && this.epg.events[this.currentVerticalIndex]) {
      this.renderNextEvents(direction);
    }
  }
  ...
  calculateVerticalIndexToChannel(direction){
    var index = this.currentVerticalIndex;
    var channelsShown = document.getElementsByClassName("epg-channel-item-id");
    channelsShown = Array.from(channelsShown);
    var channelsShownValues = [];
    if(channelsShown.length > 1){
      channelsShown.map((value) => {
        channelsShownValues.push(value.innerHTML);
      })
    }
    const channelToCompare = channelsShownValues.length == 6 ? channelsShownValues[1] : channelsShownValues[0]
       this.epg.data.map((value, key) => {
      if(value[0].number === channelToCompare && direction == this.upDirection && this.epg.data[key - 1]){
        index = key - 1;
      }else if(value[0].number === channelToCompare && direction == this.downDirection && this.epg.data[key + 1]){
        index = key + 1;
      }
    })
    return index;
  ...
  renderChannels(direction) {
    // this.currentVerticalIndex = direction ? this.calculateVerticalIndexToChannel(direction) : this.currentVerticalIndex;
    if (this.epg && this.epg.channels && this.epg.channels[this.currentVerticalIndex]) {
      this.channelsContainer.innerHTML = this.epg.channels[this.currentVerticalIndex].join('');
      ...
      channels.forEach(item => {
        const group_id = item.getAttribute('data-group-id')
        const canPlayChannel = ChannelSingleton.canPlayChannel(group_id);
        if (!canPlayChannel) {
          item.classList.add("channelBlock");
        }
      })
    }
  }

  calculateHorizontalIndex(direction){
    var indexHorizontal = this.currentHorizontalIndex;
    var hour = document.getElementsByClassName("epg-hour");
    hour = Array.from(hour);
    if(this.epg && this.epg.schedules && direction && hour.length > 0){
      this.epg.schedules.map((hours, index)=>{
        hours.map((eachHour) => {
          if(hour[hour.length - 1] && (""+moment(eachHour).format('DD/MM HH.mm')+"hs.") == hour[hour.length - 1].innerText && direction == this.rightDirection){
             indexHorizontal = index + 1;
          }
          if(hour[hour.length - 4] && (""+moment(eachHour).format('DD/MM HH.mm')+"hs.") == hour[hour.length - 4].innerText && direction == this.leftDirection){
            indexHorizontal = index - 1;
          }
        })
    })
  ...
  renderSchedules(direction = null) {
    let nextSchedules = '';
    var indexHorizontal = this.calculateHorizontalIndex(direction);

    if (this.epg && this.epg.schedules && this.epg.schedules[indexHorizontal]) {
      this.epg.schedules[indexHorizontal].forEach((current, index) => {
        ...
        nextSchedules = `${nextSchedules}<div class=${`epg-hour`} data-time-index=${index} style='width: ${width}px;'>${moment(current).format('HH.mm')}</div>`;
      });
  ...
  calculateVerticalIndex(){
    ...
    const channelToCompare = channelShowsValues.length == 6 ? channelShowsValues[1] : channelShowsValues[0]
        this.epg.data.map((value, key) => {
  ...
  renderEvents(direction = null) {
    ...
    if (this.now().isBetween(this.memory_visible.horizontal.start, this.memory_visible.horizontal.end)) {
      const diff = this.now().diff(this.memory_visible.horizontal.start, 'm');
      const offset=this.eventContainer.offsetLeft === 0? 153 : this.eventContainer.offsetLeft; // caso para el render inicial pone la linea de tiempo al principio, es 153 por que this.eventContainer.offsetLeft siempre es 153 excepto en el primer render es 0
    ...
    if (this.epg && this.epg.events && this.epg.events[this.currentVerticalIndex]) {
      //console.info("RDGV2 [EpgPostales] renderEvents 2 this.epg: ", this.epg);
      this.epg.events[this.currentVerticalIndex].forEach((row, index) => {
        console.log("LUCHO item", row)
        //console.info("RDGV2 [EpgPostales] renderEvents 2 currentHorizontalIndex: ", (row[this.currentHorizontalIndex] ? ' con valor' : 'undefined*****'));
        nextEvents = `${nextEvents}<div id=${`row-${index}`} class='epg-events-row' style='width: ${this.rowWidth}px;'>${row[this.currentHorizontalIndex]}</div>`;
      }, this);
      const current = this.getFocusedData();
      nextFocus = !direction ? '.epg-events-container .focusable' : this.getDirectionType(direction) == this.horizontalDirection ? `#${current.row.id} .focusable` : direction == this.upDirection ? `.epg-events-container .epg-events-row:last-of-type .focusable` : `.epg-events-container .focusable`;
      this.eventContainer.innerHTML = nextEvents;

      const rowBlok = document.querySelectorAll(
        ".epg-events-container .epg-events-row .epg-events-block"
      );

      rowBlok.forEach(item => {
        const group_id = item.getAttribute('data-group-id')
        const canPlayChannel = ChannelSingleton.canPlayChannel(group_id);
        if (!canPlayChannel) {
          item.classList.add("rowBlock");
        }
      })
    }
    ...
    if (Utils.isModalHide()) {
      ...
      if(this.eventContainer.parentNode && this.eventContainer.parentNode.parentNode && this.eventContainer.parentNode.parentNode.parentNode &&
        this.eventContainer.parentNode.parentNode.parentNode.parentNode && this.eventContainer.parentNode.parentNode.parentNode.parentNode.style &&
        this.eventContainer.parentNode.parentNode.parentNode.parentNode.style.display=='block')
      {
        nextFocus=' '+nextFocus
        let toArray=[...this.eventContainer.parentNode.parentNode.parentNode.parentNode.classList];
        toArray.forEach(x=>{nextFocus="."+x+nextFocus})
      }
      window.SpatialNavigation.focus(nextFocus);
    }
  }
  ...
  renderNextEvents(direction) {
    ...
    if (this.isHorizontalDirection(direction)) {
      if (direction == this.rightDirection) {
        this.memory_visible.horizontal.start = moment(this.memory_visible.horizontal.end);
        this.memory_visible.horizontal.end = moment(this.memory_visible.horizontal.start).add(this.props.visibleTime, 'hours');
      } else {
        this.memory_visible.horizontal.end = moment(this.memory_visible.horizontal.start);
        this.memory_visible.horizontal.start = moment(this.memory_visible.horizontal.end).subtract(this.props.visibleTime, 'hours');
      }
    } else {
      if (direction == this.downDirection) {
        nextFrom = this.memory_visible.vertical.start + this.props.visibleRows;
        this.memory_visible.vertical.start = nextFrom > this.total ? nextFrom - this.total : nextFrom;
      } else {
        nextFrom = this.memory_visible.vertical.start - this.props.visibleRows;
        this.memory_visible.vertical.start = nextFrom < 0 ? this.total - Math.abs(nextFrom) : nextFrom;
      }
    }
    //Renders
    if (this.getDirectionType(direction) == this.verticalDirection && this.channelsContainer) {
      this.renderChannels(direction);
    }
    else if (this.schedulesContainer) {
      this.renderSchedules(direction);
    }
    if (this.eventContainer) {
    ...
    if (this.isGoingToFetch || this.forceFill) {
      const loadIn = 2;
      if (!this.isRequesting && direction == this.rightDirection && this.epg.events[this.currentVerticalIndex][0].length == this.currentHorizontalIndex + loadIn) {
        if(this.changeFirstChannelEPG){
          this.memory_range.vertical.start = this.props.from;
        }
        this.memory_range.horizontal.start = moment(this.memory_visible.horizontal.start).add(loadIn * this.props.visibleTime, 'hours');
        this.memory_range.horizontal.end = moment(this.memory_range.horizontal.start).add(this.props.horizontalOffset * this.props.visibleTime * 2 + this.props.visibleTime, 'hours');
       this.loadData(direction);
      } else if (!this.isRequesting && direction == this.leftDirection && this.currentHorizontalIndex == loadIn) {
        this.memory_range.horizontal.end = moment(this.memory_visible.horizontal.start).subtract(loadIn * this.props.visibleTime, 'hours');
        this.memory_range.horizontal.start = moment(this.memory_range.horizontal.end).subtract(this.props.horizontalOffset * this.props.visibleTime * 2 + this.props.visibleTime, 'hours');
        this.loadData(direction);
      } else if (!this.isRequesting && direction == this.downDirection && this.epg.events.length == (this.currentVerticalIndex + 1)) {
        nextFrom = this.memory_visible.vertical.start - this.props.visibleRows;
        this.memory_range.vertical.start = nextFrom > 0 ? nextFrom : this.total - Math.abs(nextFrom);
        this.memory_range.horizontal.start = moment(this.memory_visible.horizontal.start).subtract(this.props.horizontalOffset * this.props.visibleTime, 'hours');
        this.memory_range.horizontal.end = moment(this.memory_visible.horizontal.end).add(this.props.horizontalOffset * this.props.visibleTime, 'hours');
        this.loadData(direction);
      } else if (!this.isRequesting && direction == this.upDirection && this.currentVerticalIndex == 0) {
        nextFrom = this.memory_visible.vertical.start - this.props.verticalOffset;
        this.memory_range.vertical.start = nextFrom > 0 ? nextFrom : this.total - Math.abs(nextFrom);
        this.memory_range.horizontal.start = moment(this.memory_visible.horizontal.start).subtract(this.props.horizontalOffset * this.props.visibleTime, 'hours');
        this.memory_range.horizontal.end = moment(this.memory_visible.horizontal.end).add(this.props.horizontalOffset * this.props.visibleTime, 'hours');
        this.loadData(direction);
  ...
  loadData(direction, customParams = {}) {
    ...
    const from = (this.fromNewRequest && direction == this.downDirection) ? this.fromNewRequest : this.memory_range.vertical.start;
    const quantity = this.getQuantity();
    const infinite_fix = from + quantity > this.total ? from + quantity - this.total : '';

    this.changeFirstChannelEPG = false;
    ...
    //     if (direction == this.downDirection) {
    //       this.currentVerticalIndex = (this.props.visibleRows === 1)
    //         ? this.props.visibleRows
    //         : this.currentVerticalIndex - (this.props.verticalOffset / this.props.visibleRows);
    //     } else {
    //       this.currentVerticalIndex = (this.props.visibleRows === 1)
    ...
    //     }
        ...
        this.epg.schedules = direction == this.rightDirection ? [...this.epg.schedules, ...this.initSchedules()] : [...this.initSchedules(), ...this.epg.schedules];

        this.epg.events = this.epg.events.map((page, i) => {
          return page.map((channels, cIndex) => {
            const next = events[i][cIndex];
            return direction == this.rightDirection ? [...channels, ...next] : [...next, ...channels];
          }, this);
        }, this);
        ...
        this.epg.data = this.epg.data.map((page, i) => {
          return page.map((channels, cIndex) => {
            ...
            const events = direction == this.rightDirection ? [...channels.events, ...next.events] : [...next.events, ...channels.events];
  ...
  componentWillReceiveProps(newProps) {
    ...
    if(!newProps.fromMiniEpg) {
      console.log('[EPEGE] epgpostales componentWillReceiveProps, no es mini willreceive nextProps: ', newProps);
    }
    if(newProps.changeByFilter){
      this.isRequesting = false;
    }
    ...
    */
    ...
    if(!newProps.fromMiniEpg && newProps.fullToHide === true && this.props.node_id !== newProps.node_id) {
      console.log('IPIGI fullTohide en postales willreceive, props, newProps: ', this.props, newProps);
      let y = 0;
      this.reset();
      this.firstLoad({
      ...
      if(this.props.node_id !== newProps.node_id
        || this.props.from !== newProps.from
        || newProps.forceUpdate
        || (this.props.from === newProps.from && this.currentVerticalIndex !== 0)
      ) {
        console.log('IPIGI fullToShow en postales willreceive, props, newProps: ', this.props, newProps);
        this.reset();
        this.firstLoad({
          node_id: newProps.node_id,
          from: newProps.from,
  ...
  componentDidUpdate() {
    ...
    if(this.props.changeFirstChannelEPG){
      this.changeFirstChannelEPG = true;
    }
  ...
  updatingTime(forceUpdate = false, nextProps = null) {
    ...
    if(time.minutes%(Metadata.get('epg_interval_time',5)) === 0 || forceUpdate) {
      // Que no lance la llamada si viene filtro (esto viene del willreceive y ahi ya hace el firsload con el filtro)
      if(Utils.isNotNullOrNotUndefined(nextProps) && Utils.isNullOrUndefined(nextProps.node_id)) {
  ...
  isHandleUpdatingTime() {
    ...
    return !checkStatusEPG && !checkStatusCoverFlowCH && !LayersControl.isUXVisible() && isMiniEpg;
  }

  eventContainerUpdater() {
    if(this.timerEventContainerUpdater) clearTimeout(this.timerEventContainerUpdater);
    if(this.isHandleUpdatingTime()) {
      // console.log('[dann] setTimeout set');
      this.timerEventContainerUpdater = setTimeout(this.updatingTime, this.timerUpdaterMin);
    }
  }

  updatingTimeAfterHideEpg(newProps) {
    const { isMiniEpg, checkStatusEPG } = this.props;
    if(checkStatusEPG !== newProps.checkStatusEPG && (!newProps.checkStatusEPG || newProps.receiveOK) && !isMiniEpg) {
      if(this.timerEventContainerUpdater) clearTimeout(this.timerEventContainerUpdater);
  ...
  render() {
    ...
    if (this.state.showVCard) {
      ...
      if (channel && event) {
        ...
        this.props.goToCard(`/epg/${channel.group_id}?eventData=${encodeURI(eventData)}`);
      /*  return <Redirect
    ...
    return (
      <div>
        {this.props.isPlayingLiveAsFull && Utils.getCoverFlowVisibilityFromMetadata().enable && <CoverFlow
          from='EPG POSTALES'
        ...
        <div className="epg-main">
          ...
          <div className="epg-channels-container" ref={(div) => this.channelsContainer = div}
               dangerouslySetInnerHTML={{__html: this.state.renderedChannels.join('')}}/>
